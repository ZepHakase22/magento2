...
namespace Solwin\Megamenu\Model\Css;
...
class Generator {
    ...
    \Solwin\Megamenu\Helper\Css $cssconfigData, \Magento\Framework\Registry $coreRegistry, \Magento\Store\Model\StoreManagerInterface $storeManager, \Magento\Framework\View\LayoutInterface $layoutManager, \Magento\Framework\Message\ManagerInterface $messageManager
    ) {
    ...
    protected function generateStoreCss($type, $storeId) {
        ...
        $str4 = 'megamenu/css/' . $type . '.phtml';
        $this->_coreRegistry->register('cssgen_store', $storeCode);

        try {
            $block = $this->_layoutManager->createBlock('Solwin\Megamenu\Block\Template')->setData('area', 'frontend')->setTemplate($str4)->toHtml();
